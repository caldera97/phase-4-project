...
function PostForm({login, posts, setPosts}) {
  ...
  function handlePostSubmit(e) {
    ...
    fetch("http://localhost:3000/posts", {
      ...
      .then((resp) => {
        if (resp.ok) {
          resp.json().then((newPost) => setPosts((prevState) => [...prevState, newPost]));
  ...
  return (
    <div id='post-form'>
      <form onSubmit={handlePostSubmit}>
        <textarea max-length='400' className='post-form-input' id='post-input' placeholder='Share your thoughts'/>
        <div id='post-buttons'>
